...
open class KastreeWriterFixed (
            ...
            v?.apply {
                when (this) {
                    ...
                    is Node.Expr.BinaryOp -> {
                        ...
                        val noSep = oper is Node.Expr.BinaryOp.Oper.Token && (oper as Node.Expr.BinaryOp.Oper.Token).token.let {
        ...
        protected fun Node.childTypeConstraints(v: List<Node.TypeConstraint>) = this@KastreeWriterFixed.also {
        ...
        protected fun Node.WithAnnotations.childAnns(sameLine: Boolean = false) = this@KastreeWriterFixed.also {
        ...
        protected fun Node.WithAnnotations.childAnnsBeforeExpr(expr: Node.Expr) = this@KastreeWriterFixed.also {
            if (anns.isNotEmpty()) {
        ...
        protected fun Node.WithModifiers.childModsBeforeType(ref: Node.TypeRef) = this@KastreeWriterFixed.also {
            if (mods.isNotEmpty()) {
        ...
        protected inline fun Node.children(vararg v: Node?) = this@KastreeWriterFixed.also { v.forEach { visitChildren(it) } }

        // Null list values are asterisks
        protected fun Node.bracketedChildren(v: List<Node?>, appendIfNotEmpty: String = "") = this@KastreeWriterFixed.also {
            if (v.isNotEmpty()) {
                append('<')
        ...
        protected fun Node.childrenLines(v: Node?, extraMidLines: Int = 0, extraEndLines: Int = 0) =
                this@KastreeWriterFixed.also { if (v != null) childrenLines(listOf(v), extraMidLines, extraEndLines) }
        ...
        protected fun stmtRequiresEmptyBraceSetBeforeLineEnd(v: Node?, next: Node?): Boolean {
            ...
            if (v !is Node.Stmt.Decl || v.decl !is Node.Decl.Structured || (v.decl as Node.Decl.Structured).members.isNotEmpty() ||
                    (v.decl as Node.Decl.Structured).form != Node.Decl.Structured.Form.CLASS) return false
            if (next !is Node.Stmt.Expr || (next.expr !is Node.Expr.Paren &&
                            (next.expr !is Node.Expr.Annotated || (next.expr as Node.Expr.Annotated).expr !is Node.Expr.Paren))) return false
            return true
        }
        ...
        companion object {
            ...
            fun <T: Appendable> write(v: Node, app: T, extrasMap: ExtrasMap? = null) =
                    app.also { KastreeWriterFixed(it, extrasMap).write(v) }
        }
    }
