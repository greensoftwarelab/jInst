...
open class ConverterFixed {
    ...
    open fun convertModifiers(v: KtModifierList?) = v?.node?.children().orEmpty().mapNotNull { node ->
        ...
        node.psi.let {
            when  {
                it is KtAnnotationEntry -> Node.Modifier.AnnotationSet(
                        target = it.useSiteTarget?.let(::convertAnnotationSetTarget),
                        anns = listOf(convertAnnotation(it))
                ).map(it)
               it  is KtAnnotation -> convertAnnotationSet(it)
               it is PsiWhiteSpace -> null
               it is PsiComment -> null
                else -> when (node.text) {
    ...
    open class WithExtras : ConverterFixed(), ExtrasMap {
    ...
    companion object : ConverterFixed() {
